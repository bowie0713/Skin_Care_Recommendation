...
First, we combined each of our each individual scraped data. From there, we filter out
all the rows/products that have `NA` values from the column Skin Type, Skin Concerns, and
Prodcut Rating because this implies that the products are not skincare products. 
...
We did some data cleaning for our dataset to remove the `$` sign for our `Product Price`
column and convert it to numeric columns. We also round all the `Product Rating`
numbers so that to ensure data consistency. 
